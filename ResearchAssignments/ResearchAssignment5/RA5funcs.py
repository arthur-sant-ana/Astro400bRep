...
def PositionvsDispersion(filenumber, window_size):
    ...
    # If you want a moving window dispersion (e.g. local standard deviation), define a window size:
    # Use a moving window over radius to compute local dispersion
    # First, sort by radius
...
def AvgDisp(start, end, window_size):
    ...
    for index in range(start, end, 5):
        dispersions.append(np.mean(PositionvsDispersion(index, window_size)))
        index+=5
